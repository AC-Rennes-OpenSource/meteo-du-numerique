...
class Utils {
  ...
  static DateTime? getLatestUpdate<T>(List<T> items, DateTime? Function(T) getUpdateDate) {
    if (items.isEmpty) return null; // Retourne null si la liste est vide
    return items
        .map(getUpdateDate)
        .whereType<DateTime>() // Filtrer uniquement les DateTime non null
        .reduce((a, b) => a.isAfter(b) ? a : b); // Trouver la date la plus récente
  }
